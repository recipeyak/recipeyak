  ...
  render () {
    return (
      <div className="grid-entire-row d-grid grid-week">
        { ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(x => <div key={x} className="day-of-week">{x}</div>) }
      </div>
    )
  }
}
  ...
  render () {
    ...
    <section>
      ...
      <div className="d-grid grid-week">
          <MonthWeekTitles/>
          { Array(daysFromSunday(this.props.date)).fill(0).map((_, i) => <div key={i}></div>) }
          ...
          { daysOfMonth(this.props.date).map(date => (
            <Day
              ...
              highlight={isWithinRange(date, startOfDay(this.props.startDay), endOfDay(this.props.endDay))}
              endDate={isSameDay(date, this.props.startDay) || isSameDay(date, this.props.endDay)}
            />
          ))}
      </div>
