...
const Invites = ({ loading, invites, decline, accept, accepting, declining }) => {
  if (loading) {
    return <p className="text-muted fs-3 align-self-center">Loading...</p>
  }

  if (invites.length === 0) {
    return <p className="text-muted fs-3 align-self-center">No new notifications.</p>
  }
        ...
        const InviteButtons = () => {
          ...
          return (
            <div className="d-flex justify-space-between align-items-center">
              <a onClick={() => decline(id)} className="text-muted">Decline</a>
              <ButtonPrimary loading={declining || accepting} onClick={() => accept(id)} className="is-small">Accept</ButtonPrimary>
            </div>
          )
        }
