...
const MetaData = ({
  ...
  const _author = isValid(author)
    ? <span>By <b className="cursor-pointer" >{author}</b> </span>
    : null
  const _source = isValid(source)
    ? <span>from <b className="cursor-pointer" >{source}</b> </span>
    : null
  const _servings = isValid(servings)
    ? <span>creating <b className="cursor-pointer" >{servings}</b> </span>
    : null
