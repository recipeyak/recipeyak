...
def add_creator_to_invite(apps, schema_editor):
    ...
    for invite in Invite.objects.all():
        if invite.creator is None:
            first_admin = invite.membership.team.membership_set.filter(is_active=True, level='admin').first()
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('core', '0042_invite_creator'),
    ]

    operations = [
        migrations.RunPython(add_creator_to_invite),
    ]
